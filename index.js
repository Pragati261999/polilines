...
function decode(encoded) {
  ...
  for (; i < decoder.length; ) {
    ...
    if (thirdDim) {
      const deltaZ = toSigned(decoder[i + 2]) / factorZ;
      lastZ += deltaZ;
      res.push({ lat: lastLat, lng: lastLng, z: lastZ });
      i += 3;
    } else {
...
function encode({
  ...
  let lastLng = Num(0);
  let lastZ = Num(0);
  polyline.forEach((location, index) => {
    const lat = Num(Math.round(location.lat * multiplierDegree));
    encodedCoords[`lat${index}`] = encodeScaledValue(lat - lastLat);
    lastLat = lat;

    const lng = Num(Math.round(location.lng * multiplierDegree));
    encodedCoords[`lng${index}`] = encodeScaledValue(lng - lastLng);
    lastLng = lng;

    if (thirdDim) {
      const z = Num(Math.round(location.z * multiplierZ));
      encodedCoords[`z${index}`] = encodeScaledValue(z - lastZ);
      lastZ = z;
    }
  });
...
app.post("/decode", async (req, res) => {
  ...
  //  const ee = encode(polyline)
  //  console.log(ee);
  const dd = decode(polyline);
  res.send(dd);
});
app.post("/encode", async (req, res) => {
  const polyline = req.body.polyline;
  const ee = encode({ polyline }); // Pass polyline as an array
  res.send({ status: true, polyline: ee });
});
